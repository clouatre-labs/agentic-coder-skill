...
```text
agentic-coder-skill/
├── skills/coder/SKILL.md          # the pipeline spec + versioned changelog (v3.13.1)
├── agents-shared/                 # harness-agnostic agent bodies — the edit source
│   └── coder-{scout,guard,build,check}.md
├── tools/agents/                  # per-harness frontmatter templates (model, tools, effort)
│   └── {pi,claude}-coder-<role>.yaml
├── agents/                        # GENERATED — never hand-edit; CI fails on drift
│   ├── pi/coder-<role>.md
│   └── claude/coder-<role>.md
├── scripts/
│   └── generate-coder-agents.sh   # --write regenerates, --check exits 1 on drift
├── githooks/                      # commit-msg, pre-commit, pre-push, post-checkout
└── docs/                          # developer setup + repo standards
```

*Figure 2: Project structure. Solid boxes are edit sources; `agents/` is a build
artifact regenerated by `scripts/generate-coder-agents.sh`.*
...
*Figure 3: Agent generation — one shared body, two frontmatter templates, two
...
*Figure 4: Handoff data flow across a complex-tier session — each arrow is a JSON
...
|---|---|---|
| bash | 3.2+ | `scripts/generate-coder-agents.sh`, githooks |
| jq | 1.6+ | handoff read/write (`jq -c .` compact form) |
| gzip | any | handoff degeneracy gate (`gzip -9` ratio test) |
| `gh` CLI | 2.x | issue/PR operations in the pipeline |
| markdownlint-cli2 | latest | Markdown lint (CI) |
| shellcheck | latest | githooks lint |
| `typesafe-judge` | contract in SKILL.md | judge-assisted classification + HYBRID gate (lives in clouatre/dotfiles) |

*Table 5: Tooling requirements. The skill itself has no compiled artifacts; these
are runtime dependencies of the pipeline and repo lint.*
